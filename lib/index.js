...
var CONFIGURATION = {
  ...
  bodyBlacklist: [], // These need to be lowercase
...
function buildHash(url, args) {
  ...
  if (args) {
    ...
    if (args.body) {
      json.body = {}
      var bodyKeys = Object.keys(args.body)
      for (var index in bodyKeys) {
        var key = bodyKeys[index]
        if (CONFIGURATION.bodyBlacklist.indexOf(key.toLowerCase()) < 0) {
          json.body[key] = args.body[key]
        }
      }
    }
